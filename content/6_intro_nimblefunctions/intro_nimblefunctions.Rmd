...
*nimbleFunctions* are at the heart of NIMBLE. They are the way that algorithms are implemented. They can also be used for
 ...
 - user-defined MCMC samplers (Module 8), and
 - compiling parts of R (not shown), without using a model.
...
Let's review how one can use nimble to understand the structure of a graphical model by looking back at Module 2.
...
- `run` and other methods can be run uncompiled (in R) or compiled (via C++).
    - Typically "actual algorithm" repeated calculations such as MCMC sampler updates.
    - Can operate models (calculate, simulate, getParam, etc.).
...
objective <- nimbleFunction(
    setup = function(model, nodes) {
        calcNodes <- model$getDependencies(nodes)
        cat("The calcNodes are ", paste0(calcNodes, sep = ','), "\n")
...
cObjective <- compileNimble(rObjective, project = regrModel)   # now compile the nimbleFunction
```
...
In the optimization, we weren't careful about avoiding negative standard deviations.

Modify the nimbleFunction run function to work on the log scale for `sigma`, by providing a second argument that indicates which elements are being provided on the log scale.
...
objective <- nimbleFunction(
    ...
    run = function(par = double(1), logscale = ???) {
        returnType(double(0))
        if(length(par) != n)
           stop("Input length does not match number of parameter elements.")
        values(model, nodes) <<- par       # <<- assignment into non-local (nf) variables 
