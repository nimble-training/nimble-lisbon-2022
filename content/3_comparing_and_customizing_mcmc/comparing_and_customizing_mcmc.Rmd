...
plot(DEsamples[,"length_coef[1]"], type = 'l', ylab = "length_coef[1]") ## Happens to mix well
plot(DEsamples[,"sex_int[1]"], type = 'l', ylab = "sex_int[1]")    ## Doesn't mix as well
```
...
The `effectiveSize` function of package `coda` gives one estimator of ESS.  (The `mcmcse` and `rstan` packages have other estimators.)
...
- If some parameters are mixing very well and others very poorly, it's not clear that you will have acceptable mixing for the quantities of interest.

Another ESS measure we could use is multivariate ESS:
...
Vats et al. (2019) Biometrika (https://doi.org/10.1093/biomet/asz002)
...
$\theta_{-1}$ = current values of all other dimensions (fixed).

Model calculations are $[Y, \theta] = [Y, (\theta_1', \theta_{-1})]$.

The needed ratio for many methods is
\[
\frac{[Y, (\theta_1', \theta_{-1})]}{[Y, (\theta_1, \theta_{-1})]} = \frac{(\mbox{Terms using } \theta_1')(\mbox{Terms not using } \theta_1')}{(\mbox{Terms using } \theta_1)(\mbox{Terms not using } \theta_1)} = \frac{\mbox{Terms using } \theta_1'}{\mbox{Terms using } \theta_1} 
\]
...
Some methods also use derivatives of $[Y, \theta]$.  (These are about to be released in nimble, and are of course available in Stan, PyMC, numpyro, etc.)


# Gibbs (conjugate) samplers


- Possible when we can write $[\theta_1 | \theta_{-1}, Y]$ analytically.
...
- How to accept or reject $\theta_1'$?
     - Calculate ratio of $[Y, (\theta_1', \theta_{-1})] / [Y, (\theta_1, \theta_{-1})]$ (using only needed terms). 
...
- Computational cost is either 
     - two evaluations of $[Y, (\theta_1', \theta_{-1})]$ (only the parts that depend on $\theta_1$), or
     - one evaluation of $[Y, (\theta_1', \theta_{-1})]$ (ditto) and some copying to save previous values.
- How to choose $\nu$? 
     - By "adaptation".  The algorithm increases or decreases $\nu$ to achieve theoretically derived optimal accpetance rate.  
- Remember that the target distribution may change on the next iteration because $\theta_{-1}$ may have been updated.
...
- Computational cost can be *many* evaluations of $[Y, (\theta_1, \theta_{-1})]$ (again, only the necessary terms).
...
- May not work well in many dimensions.
...
# Other samplers (about to be released in nimble)
...
# Problematic posterior correlation

Clearly the sex intercepts mix most poorly. Perhaps surprisingly they are positively correlated.

```{r}
par(mfrow = c(1,2))
ts.plot(DEsamples[ , 'sex_int[1]'], ylab = 'sex_int[1]', main = 'sex_int[1]')
ts.plot(DEsamples[ , 'sex_int[2]'], ylab = 'sex_int[2]', main = 'sex_int[2]')
```

One might think that constraining the farm effect mean to zero would eliminate mixing related to trading off between the intercept(s) and the random effects, but:

```{r}
par(mfrow = c(3,1), mgp = c(1.8,.7,0), mai = c(.5, .4, .3, .1))
ts.plot(DEsamples[ , 'sex_int[1]'], ylab = 'sex_int[1]', main = 'sex_int[1]')
ts.plot(DEsamples[ , 'sex_int[2]'], ylab = 'sex_int[2]', main = 'sex_int[1]')
farmEffectCols <- grep('farm_effect', colnames(DEsamples))
ts.plot(rowMeans(DEsamples[ , farmEffectCols]), ylab = 'random effect mean', main = 'farm_effect mean')
```

We'll focus on these issues in the next module.
