...
rnormmix2 <- nimbleFunction(
  run = function(n = integer(), 
    ...
    ind <- rbinom(1, 1, prob) + 1  # dbern/rbern not available for compilation
...
- In compiled mode, non-scalar arguments are passed by reference (i.e., C-like behavior).
- In uncompiled mode, they are passed by copy (i.e., R-like behavior)
...
Here is an example.  Suppose the model's total log probability is `-Inf` or `NaN`.  This occurs when something is wrong:

 - Sometimes initial values are invalid.
 - Sometimes a user-defined function has a bug.
 - Sometimes model logic is wrong.

To investigate, we can step into `expcov` calculations as it is used from the model:
...
In the simple 4-observation regression example from earlier, all the observations depend on each parameter (`intercept`, `slope` or `sigma`).

Therefore, it would be more efficient to specify `y[1:4]` as coming from a single distribution:
