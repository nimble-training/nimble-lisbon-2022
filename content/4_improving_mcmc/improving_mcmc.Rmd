...
Some strategies for improving MCMC
 ...
 - Customize sampler choices. For example:
...
```{r, echo=FALSE, val=doComparisons&has_compareMCMCs}
...
(If we want inference on $\mu_i, i=1,\ldots,n$ we need to sample the latent states conditional on the data and the MCMC draws of $\mu_0, \sigma_0^2, \sigma^2$. We'll work on this in Module 9.)
...
In Stan, since HMC can't handle discrete parameters, one must use the marginalized version.
